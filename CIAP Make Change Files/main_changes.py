...
class SerialUtility:
    ...
    def paste(self):
        try:
            clipboard_text = self.root.clipboard_get()  # Use self.root to get clipboard text
            self.log_console.insert(tk.END, clipboard_text + '\n')  # Paste it into the console
            self.log_console.yview(tk.END)  # Scroll to the bottom

            # Send the pasted commands to the device if the serial port is open
            if self.serial_port and self.serial_port.is_open:
                for line in clipboard_text.strip().splitlines():
                    command = line.strip()
                    if command.startswith('*'):
                        self.serial_port.write((command + '\n').encode())  # Send command
                        self.insert_log(f"Sent command: {command}")  # Log the command sent
                        self.wait_for_device_response()  # Optionally wait for the response
                    else:
            ...
            try:
                ...
                while True:
                    if self.serial_port.in_waiting > 0:
                        response = self.serial_port.readline().decode('utf-8').strip()
                        self.insert_log(f"Device Response: {response}")
                        # You can implement your exit condition based on the response here
                        break
